...
int main(void)
{
    ...
    while (!gameover)
    {
        do
        {
            ...
            printf("Please input the location (row col): ");
            scanf("%i %i", &row, &col);
        } while (row > n || row < 1 || col > n || col < 1 || board[row][col] != 0); // 修改范围检查为从1开始
    ...
    printf("****************************************************************************\n");
    printf("GAMEOVER! Winner is player %i!\n", (player == 1) ? 2 : 1);
}
...
int wincheck(int row, int col, int player, int n)
{
    ...
    for (int i = 1; row + i < n && board[row + i][col] == player; i++)
        count++;
    for (int i = 1; row - i >= 1 && board[row - i][col] == player; i++) // 修改为 row - i >= 1
        count++;
    if (count >= 5)
        return 1;
    ...
    for (int i = 1; col + i < n && board[row][col + i] == player; i++)
        count++;
    for (int i = 1; col - i >= 1 && board[row][col - i] == player; i++) // 修改为 col - i >= 1
        count++;
    if (count >= 5)
        return 1;
